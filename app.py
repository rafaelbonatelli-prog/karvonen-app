...
if arquivos_tcx:
    with st.spinner("Processando o histórico de treinos..."):
        ...
        if historico:
            ...
            # --- O TRUQUE PARA NÃO SUMIR O GRÁFICO ---
            buf = io.BytesIO()
            fig.savefig(buf, format="png", dpi=300, bbox_inches='tight') # Tiramos a "foto"
            buf.seek(0)
            
            st.image(buf, use_container_width=True) # Mostramos a foto na tela (isso não falha)
            
            st.download_button(
                label="⬇️ Baixar Gráfico (PNG)",
                data=buf, # O botão baixa a exata mesma foto
                file_name=f"Grafico_Karvonen_{treino_dados['Data'].replace('/', '-')}.png",
                mime="image/png"
            )
